...
const StoryRing = dynamic(
  () => import("@/components/stories/StoryRing").then((mod) => ({ default: mod.StoryRing })),
  { ssr: false },
);
...
export default function RestaurantViewClient({
  city,
  slug,
  restaurantId,
  restaurant: r,
            ...
            <div className="flex flex-col gap-0.5 md:gap-1 px-0.5 mt-0.5 md:mt-1">
              ...
              {todayTiming && (
                <div className="flex items-center gap-2.5 group cursor-default">
                  <div
                    className={`w-6 h-6 md:w-7 md:h-7 rounded-lg ${todayTiming.isClosed ? "bg-red-50 text-red-500 border-red-200" : "bg-gray-100 text-black border-gray-200"} flex items-center justify-center transition-all shrink-0 border`}
                  >
                    <Clock className="w-3.5 h-3.5 md:w-4 md:h-4" />
                  </div>
                  <span
                    className={`text-[11px] md:text-[13px] font-bold ${todayTiming.isClosed ? "text-red-700" : "text-zinc-900"}`}
                  >
                    {todayTiming.isClosed ? (
                      ...
                      <>
                        ...
                        <span className="text-black font-extrabold">
                          {to12h(todayTiming.open)} - {to12h(todayTiming.close)}
                        </span>
                      </>
                    )}
                  </span>
